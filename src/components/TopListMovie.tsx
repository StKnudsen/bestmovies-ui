...
const TopListMovie = ( props: IProps ) => {
    ...
    return (
        <Card 
            key={movieId} 
            sx={{ backgroundImage: `url(${backdrop})`, backgroundSize: "cover", mt: 2 }}
        >
                <Box sx={{ backgroundColor: "rgb(248, 248, 255, .8)", display: 'flex' }}>
                    <Typography variant="h3" px={3} alignSelf={'center'}>{props.index + 1}</Typography>
                    <CardMedia
                        component="img"
                        image={poster}
                    ...
                    <Box display={'flex'} flexDirection={'column'}>
                        <Typography variant="h5" p={2}>{movie.title}</Typography>
                        <Typography variant="body1" px={2}>{movie.overview}</Typography>
                        <Button variant="contained" sx={{ m: 2}} component={Link} to={`/movie/${movie.id}`}>
                            Go to details
                            </Button>
                    </Box>
                </Box>
